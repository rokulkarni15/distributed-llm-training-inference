#!/usr/bin/env python3
"""
DeepSpeed ZeRO-1 training: PyTorch + LoRA + DeepSpeed ZeRO-1 on 1-4 GPUs.
...
Usage:
    ...
    torchrun --nproc_per_node=2 training/train_deepspeed_zero1.py --dataset_path ./data/glaive_code_full --resume_from_checkpoint
    torchrun --nproc_per_node=3 training/train_deepspeed_zero1.py --dataset_path ./data/glaive_code_full --resume_from_checkpoint
    torchrun --nproc_per_node=4 training/train_deepspeed_zero1.py --dataset_path ./data/glaive_code_full --resume_from_checkpoint
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Llama 2 7B with LoRA + DeepSpeed ZeRO-1 (1-4 GPUs)"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./checkpoints/deepspeed_zero1",
...
def main():
    ...
    if is_main_process:
        print("\n" + "="*70)
        print(f"DEEPSPEED ZeRO-1 TRAINING: PYTORCH + LORA + DeepSpeed ({world_size} GPU{'s' if world_size > 1 else ''})")
        print("="*70)
        print("\nNOTE: Using DeepSpeed ZeRO-1 for optimizer state partitioning")
        if is_distributed:
            print(f"Expected: Memory savings + {world_size}x speedup with {world_size} GPUs")
        else:
